...
namespace Terraria.Plugins.Common
{
    public class TerrariaItems
    {
        ...
        public int GetItemTypeFromBlockType(int blockType, int objectStyle = 0)
        {
            ...
            int x = Main.maxTilesX - 1;
            int y = Main.maxTilesY - 1;

            var overwrittenTile = Main.tile[x, y];
            WorldGen.Place1x1(x, y,blockType, objectStyle);
            WorldGen.KillTile_GetItemDrops(x, y, Main.tile[x, y], out int id, out _, out _, out _);
            WorldGen.Place1x1(x, y, overwrittenTile.type);
            return id;
